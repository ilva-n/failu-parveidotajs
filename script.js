            ...
            try {
                ...
                if (failaVeids === "paraugi"){
                    for (let i = 0; i < featureCount; i++) {
                        //visiem izņemto pirmo feature pēc kārtas   
                        if ((i > 0) && (features[i].properties["Akta nr"] == features[i - 1].properties["Akta nr"]) 
                            && (features[i].geometry.coordinates[0] == features[i - 1].geometry.coordinates[0]) 
                            && (features[i].geometry.coordinates[1] == features[i - 1].geometry.coordinates[1])){
                            // ja tādas pašas koordinātas kā iepriekšējai feature, tad netaisīs jaunu rindu, bet kabinās klāt pie iepriekšējās
                            const tabbleRows = document.getElementsByTagName("tr");
                            const l = tabbleRows.length - 1
                            const lastRow = tabbleRows[l];
                            rowCells = lastRow.getElementsByTagName("td");
                            ...
                            rowCells[3].textContent += plusKokaSuga;
                                //console.log(rowCells[3].textContent);
                        } else {
